...
public class OkHttpManager extends AppCompatActivity {
     ...
     * @param mwhat
     */
    public void postFileMethod(String url, final String msg, Map<Object, Object> map, List<File> files, final Handler handler, final int mwhat) {
        MultipartBody.Builder builder = new MultipartBody.Builder().setType(MultipartBody.FORM);
        //添加文件
        if (files != null) {
            for (int i = 0; i < files.size(); i++) {
                Log.e("files[i].getName()", files.get(i).getName());

                RequestBody fileBody = RequestBody.create(MediaType.parse(getMediaType(files.get(i).getName())), files.get(i));

                builder.addFormDataPart("文件" + i, files.get(i).getName(), fileBody);
            }
        }
        //添加参数
        if (map != null) {
